...
//			i++;
			try {
				...
				DungeonLog.errorln("error2:" + mob.getName());
				e.printStackTrace();
				Bukkit.broadcastMessage("error2:" + mob.getName());
			}
		}
		...
		try {
			...
			for (int i = 0; i < count; i++) {
				if (arg3.length == 1) {
					...
					mob.spawn(new Location(arg0.getBlock().getWorld(), Double.parseDouble(arg3[1]), Double.parseDouble(arg3[2]), Double.parseDouble(arg3[3])));
				}
			}
			return true;
		...
		try {
			...
			for (int i = 0; i < count; i++) {
				if (arg3.length == 1) {
					...
					mob.spawn(new Location(p.getWorld(), Integer.parseInt(arg3[1]), Integer.parseInt(arg3[2]), Integer.parseInt(arg3[3])));
				}
			}
			return true;
		} catch (Exception e) {
