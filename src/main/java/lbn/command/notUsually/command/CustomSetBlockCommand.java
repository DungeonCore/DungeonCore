...
public class CustomSetBlockCommand extends AbstractVanillaCommand {
  ...
  protected void execute2(ICommandListener paramICommandListener,
    ...
    if (paramArrayOfString.length < 4) { throw new ExceptionUsage("commands.setblock.usage", new Object[0]); }
    ...
    if (!localWorld.isLoaded(localBlockPosition)) { throw new CommandException("commands.setblock.outOfWorld", new Object[0]); }
    ...
    if (!localWorld.setTypeAndData(localBlockPosition, localIBlockData, 2)) { throw new CommandException("commands.setblock.noChange",
        new Object[0]); }
    if (j != 0) {
