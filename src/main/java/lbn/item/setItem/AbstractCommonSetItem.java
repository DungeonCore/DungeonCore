...
public abstract class AbstractCommonSetItem implements SetItemInterface {
  ...
  public boolean isWearSetItem(Player p) {
    ...
    for (Entry<SetItemPartsType, SetItemPartable> entry : getFullSetItem().entrySet()) {
      ...
      // もし装備していなければFALSE
      if (itemStackByParts == null) { return false; }
      // 別なものを装備していればFALSE
      if (!entry.getValue().isThisItem(itemStackByParts)) { return false; }
    }
    return true;
  }
  ...
  public boolean equals(Object paramObject) {
    if (paramObject instanceof SetItemInterface) { return getName().equalsIgnoreCase(((SetItemInterface) paramObject).getName()); }
    return false;
  }
