...
public class Buyer extends TheLowMerchant {
  ...
  protected void onSetItem(InventoryView inv) {
    ...
    if ((item1 == null && item2 == null) || (item1.getType() == Material.AIR && item2.getType() == Material.AIR)) {
      sendRecipeList(
          new TheLowMerchantRecipe(ItemStackUtil.getItem("売りたいアイテム1", Material.DIAMOND_AXE), ItemStackUtil.getItem("値段", Material.GOLD_INGOT)));
      return;
    }
  ...
  public List<TheLowMerchantRecipe> getInitRecipes() {
    return Arrays
        .asList(new TheLowMerchantRecipe(ItemStackUtil.getItem("売りたいアイテム1", Material.DIAMOND_AXE), ItemStackUtil.getItem("値段", Material.GOLD_INGOT)));
  }
