...
import net.l_bulb.dungeoncore.common.particle.StraightParticleData;
...
class LaserBowRunner extends LbnRunnable {
  ...
  public void run2() {
    ...
    Location add2 = new Location(add.getWorld(), add.getX() + Math.sin(Math.toRadians(rndAngle)) * rndRadius, add.getY(), add.getZ()
        + Math.cos(Math.toRadians(rndAngle)) * rndRadius);
    data.run(add2);
