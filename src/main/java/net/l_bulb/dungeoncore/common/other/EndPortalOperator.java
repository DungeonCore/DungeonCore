...
public class EndPortalOperator {
  ...
  public static void subExecute(BlockMultiPlaceEvent e) {
    // エンドポータルでないなら無視
    if (e.getBlock().getType() != Material.ENDER_PORTAL_FRAME) { return; }

    if (e.getReplacedBlockStates().size() != 10) { return; }

    // 全てEndPortalであることを確認する
    for (BlockState states : e.getReplacedBlockStates()) {
      if (states.getBlock().getType() != Material.ENDER_PORTAL && states.getBlock().getType() != Material.ENDER_PORTAL_FRAME) { return; }
    }
  ...
  public static void onClick(PlayerInteractEvent e) {
    ...
    // エンダーアイの処理
    if (item == null || item.getType() != Material.EYE_OF_ENDER) { return; }
