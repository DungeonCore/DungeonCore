...
public class RouteSearcher {
  ...
  public void startSearchingPath() {
    ...
    DungeonLogger.info(sb.toString());
  ...
  enum SearchDirection {
    UP(0, 1, 0), DOWN(0, -1, 0), RIGHT(1, 0, 0), LEFT(-1, 0, 0), FRONT(0, 0, 1), BACK(0, 0, -1);
    ...
    public boolean isInverce(SearchDirection d) {
      if (x != -1 * d.x) { return false; }
      if (y != -1 * d.y) { return false; }
      if (z != -1 * d.z) { return false; }
      return true;
    }
