...
public class CustomEffectCommand extends AbstractVanillaCommand {
  ...
  protected void execute2(ICommandListener paramICommandListener,
    ...
    if (paramArrayOfString.length < 2) { throw new ExceptionUsage("commands.effect.usage", new Object[0]); }
    ...
    if (paramArrayOfString[1].equals("clear")) {
      if (localEntityLiving.getEffects().isEmpty()) { throw new CommandException(
    ...
    try {
      ...
      if (localMobEffectList1 == null) { throw localExceptionInvalidNumber; }
  ...
  private String getEffectName(String effect) {
    if (nameMap.containsKey(effect.toUpperCase())) { return nameMap.get(effect.toUpperCase()); }
    return effect;
  }
