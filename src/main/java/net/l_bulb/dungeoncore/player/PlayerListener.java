...
public class PlayerListener implements Listener {
  ...
  public void onChangeStatusLevel(PlayerChangeStatusLevelEvent e) {
    String join = StringUtils.join(new Object[] { e.getOfflinePlayer().getName(), "の", e.getLevelType().getName(), "がレベル", e.getLevel(), "(",
        e.getNowExp(), ")になりました。" });
  ...
  public void onMoneyExp(PlayerChangeGalionsEvent event) {
    ...
    if (event.getReason() != GalionEditReason.mob_drop) {
      ...
      final String message = String.format(format, event.getTheLowPlayer().getName(), event.getAddGalions(), event.getReason().toString(), event
          .getTheLowPlayer().getGalions());
  ...
  public void onPlayerCraftCustomItemEvent(PlayerCraftCustomItemEvent event) {
    event.getPlayer().sendMessage(
        MessageFormat.format("{0}アイテム{1}[{2}]{3}を作成しました", ChatColor.GREEN, ChatColor.LIGHT_PURPLE,
