...
public class QuestViewIcon {
	...
	public ItemStack getItemStack(Quest quest) {
		...
		if (quest.isMainQuest()) {
			...
			// アイテム名追加
			ItemStackUtil.setDispName(itemStack,
					MessageFormat.format("{0}{1}{2}[メインクエスト]", ChatColor.GOLD, quest.getName(), ChatColor.RED));
		} else {
		...
		ItemStackUtil.addLore(itemStack,
				MessageFormat.format(" {0}{1}", ChatColor.WHITE, quest.getComplateCondition()));
		...
		if (status == QuestProcessingStatus.PROCESS_END && quest.getEndVillagerId() != null) {
			ItemStackUtil.addLore(itemStack,
					MessageFormat.format(" {0}{1}", ChatColor.WHITE, quest.getEndVillagerId() + "のところへ報告へ行こう"));
		}
