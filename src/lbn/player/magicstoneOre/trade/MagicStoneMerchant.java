...
public class MagicStoneMerchant implements IMerchant {
	...
	public MerchantRecipeList getOffers(EntityHuman paramEntityHuman) {
		// ダイヤ鉱石
		MerchantRecipe merchantRecipe1 = new MagicStoneMerchantRecipe(MagicStoneOreType.DIAOMOD_ORE,
				CraftItemStack.asNMSCopy(diamond));
		// レッドストーン鉱石
		MerchantRecipe merchantRecipe2 = new MagicStoneMerchantRecipe(MagicStoneOreType.REDSTONE_ORE,
				CraftItemStack.asNMSCopy(redstone));
		// 鉄鉱石
		MerchantRecipe merchantRecipe3 = new MagicStoneMerchantRecipe(MagicStoneOreType.IRON_ORE,
				CraftItemStack.asNMSCopy(iron));
		// エメラルド鉱石
		MerchantRecipe merchantRecipe4 = new MagicStoneMerchantRecipe(MagicStoneOreType.EMERALD_ORE,
...
class MagicStoneMerchantRecipeList extends MerchantRecipeList {
	 ...
	 */
	@Override
	public MerchantRecipe a(net.minecraft.server.v1_8_R1.ItemStack paramItemStack1,
			net.minecraft.server.v1_8_R1.ItemStack paramItemStack2, int paramInt) {
		// 設置したアイテムからrecipeを取得
		...
		for (int i = 0; i < size(); i++) {
			...
			if ((net.minecraft.server.v1_8_R1.ItemStack.c(paramItemStack1, localMerchantRecipe2.getBuyItem1()))
					...
					&& (((!localMerchantRecipe2.hasSecondItem()) && (paramItemStack2 == null))
							|| ((localMerchantRecipe2.hasSecondItem())
									&& (net.minecraft.server.v1_8_R1.ItemStack.c(paramItemStack2,
											localMerchantRecipe2.getBuyItem2()))
		...
		return new MagicStoneMerchantRecipe(recipe.getType(),
				CraftItemStack.asNMSCopy(MagicStoneTradeData.getRandomMagicStoneItem(recipe.getType())));
	}
}
