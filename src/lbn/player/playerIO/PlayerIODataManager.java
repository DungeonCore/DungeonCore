...
public class PlayerIODataManager {
	...
	public static void kickPlayerByLoadData(Player p) {
		...
		Message.sendMessage(p, ChatColor.RED + "PlayerDataのロードに失敗しました。\n" + ChatColor.RED
				+ "管理者(twitter:{0})まで連絡ください。\n " + "ChatColor.RED + 10秒後に自動的にログアウトします。", Config.DEVELOPER_TWITTER_ID);
		new BukkitRunnable() {
			@Override
			public void run() {
				p.kickPlayer("sorry, System cant load your game data. please tell developer (twitter:"
						+ Config.DEVELOPER_TWITTER_ID + ")\n" + "申し訳ありません。あなたのゲームデータをロード出来ませんでした。 開発者(twitter: "
						+ Config.DEVELOPER_TWITTER_ID + ")まで連絡ください。");
			}
	...
	public static void saveAsZip() {
		...
		// InOutputUtil.compressDirectory(new File(Main.dataFolder +
