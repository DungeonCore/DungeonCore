...
public class OldNormalMobSkill implements MobSkillInterface {
	...
	public OldNormalMobSkill(double damage, int fireTick, MobSkillRunnable runnable, MobSkillExcuteTimingType timing,
			...
			String targetingMethodData, int laterTick, String chainId, String skillTalk, String soundId,
			boolean isSoundTargetOnePlayer, String buffId1, boolean isTargetMobBuff) {
		this.damage = damage;
	...
	public void executeMobSkill(Entity condtionTarget, Entity mob) {
		// 対象の選び方によって実行方法を変える
		if (targetingMethod == MobSkillTargetingMethodType.DROPING_ITEM
				|| targetingMethod == MobSkillTargetingMethodType.FALLING_BLOCK) {
	...
	protected void executeFallingblockDamage(Entity condtionTarget, Entity mob) {
		...
		DamageFallingblockForMonsterSkill damageFallingblockForMonsterSkill = new DamageFallingblockForMonsterSkill(mob,
				condtionTarget.getLocation(), getMaterialById(blockId), (byte) data, speed) {
