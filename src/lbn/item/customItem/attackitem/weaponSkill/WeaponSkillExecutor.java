...
public class WeaponSkillExecutor {
	...
	public static void executeWeaponSkillOnClick(PlayerInteractEvent e, AbstractAttackItem customItem) {
		...
		CooltimeManager cooltimeManager = new CooltimeManager(player,
				new CooltimeImpl(customItem.getAttackType(), skill.getCooltime()), item);
		// クールタイムが残っていないか確認
	...
	private static void sendMessage(Player p, WeaponSkillInterface skill) {
		...
		for (Player player : players) {
			...
			if ((x * x) + (y * y) + (z * z) < 30 * 30) {
				player.sendMessage(MessageFormat.format("{0}[武器スキル] {1}{2}が{3}を発動", ChatColor.RED, ChatColor.GREEN,
						p.getName(), skill.getName()));
			}
		}
	}
