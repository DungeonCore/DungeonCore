...
public class StrengthMerchant extends TheLowMerchant {
	...
	protected void onSetItem(InventoryView inv) {
		...
		MerchantRecipeCreator merchantRecipeCreator = new MerchantRecipeCreator(dispItem1, dispItem2, theLowPlayer,
				strengthData);
	...
	public List<TheLowMerchantRecipe> getInitRecipes() {
		...
		recipes.add(
				new TheLowMerchantRecipe(ItemStackUtil.getItem("強化したい防具", Material.IRON_CHESTPLATE, "強化する防具を置いてください"),
	...
	public void onFinishTrade(TheLowMerchantRecipe recipe) {
		// お金を消費する
		theLowPlayer.setGalions(theLowPlayer.getGalions() - strengthData.getNeedMoney(),
				GalionEditReason.consume_strength);
		// Eventを発生させる
		PlayerStrengthFinishEvent playerStrengthFinishEvent = new PlayerStrengthFinishEvent(getPlayer(),
				strengthData.getNextLevel(), recipe.getResult(), strengthData.isSuccessStrength());
