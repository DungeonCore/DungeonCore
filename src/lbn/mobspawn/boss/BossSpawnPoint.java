...
public class BossSpawnPoint extends SpletSheetMobSpawnerPoint{
	...
	public int spawnMob() {
		if (boss == null) {
			DungeonLog.errorln("mob is invalid:" + mobGetter.getName());
...
//		if (entity != null && entity.isValid()) {
			...
			cancelReson = "mob is alive yet. (" + location.getBlockX() + ", " + location.getBlockY() + ", " + location.getBlockZ() + ") entity is valid:" + entity.isValid();
			return 0;
		}
	...
	private boolean existMobInNear() {
		...
		for (Entity livingEntity : spawn.getNearbyEntities(50, 10, 50)) {
			...
			if (((LivingEntity)livingEntity).getCustomName() != null && ((LivingEntity)livingEntity).getCustomName().contains(boss.getName())) {
				if (boss.getEntity() == null) {
					boss.setEntity((LivingEntity) livingEntity);
					DungeonLog.errorln(boss.getName() + "を再セットしました。");
				} else {
					DungeonLog.errorln(boss.getName() + "が存在してるのにも関わらず、システムはボスモンスターを再召喚しようとしました。 entity is dead:" + boss.getEntity().isDead());
				}
				spawn.remove();
				return true;
			}
	...
	public boolean isAliveEntity(LivingEntity entity) {
		if (entity == null) {
//			DungeonLog.printDevelopln(boss.getName() + " is null. So buss will be spawned!!");
			return false;
		}
...
//			DungeonLog.printDevelopln(boss.getName() + " is dead!!");
		...
		DungeonLog.printDevelopln(entity.getCustomName() + " is not alive. So buss will be spawned!!");
		return false;
	}
