...
public abstract class DamagedFallingBlockForPlayer extends AbstractDamageFallingblock {
	...
	public DamagedFallingBlockForPlayer(Vector direction, Location loc, Material m, ItemStack item, Player p,
			double damage, byte data) {
	...
	protected boolean damageLivingentity(Entity entity) {
		if (LivingEntityUtil.isEnemy(entity)) {
			...
			// eventを実行
			PlayerCombatEntityEvent playerCombatEntityEvent = new PlayerCombatEntityEvent(p, (LivingEntity) entity,
					item, damage);
