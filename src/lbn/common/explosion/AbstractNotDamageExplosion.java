	...
	public AbstractNotDamageExplosion(Location l, float f) {
		...
		while (iterator.hasNext()) {
			...
			if (entityhuman.e(l.getX(), l.getY(), l.getZ()) < 4096.0D) {
				((EntityPlayer) entityhuman).playerConnection.sendPacket(new PacketPlayOutExplosion(l.getX(), l.getY(),
						l.getZ(), f, Lists.newArrayList(), (Vec3D) this.b().get(entityhuman)));
			}
		}
	}
				...
				if (d7 <= 1.0D) {
					...
					if (d11 != 0.0D) {
						...
						if (!isNotDamage(entity.getBukkitEntity())) {
							wasDamaged = entity.damageEntity(DamageSource.explosion(this),
						...
						if ((wasDamaged) || ((entity instanceof EntityTNTPrimed))
							...
							if (((entity instanceof EntityHuman))
								...
								this.l.put((EntityHuman) entity, new Vec3D(d8 * d13, d9 * d13, d10 * d13));
							}
						}
					}
				}
