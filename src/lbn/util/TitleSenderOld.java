...
public class TitleSenderOld {
	...
	static TitleSenderOld sender = null;

	public static TitleSenderOld getInstance() {
		if (sender == null) {
			sender = new TitleSenderOld();
		}
		return sender;
	}

	private TitleSenderOld() {
		try {
			// 一時的に使用する変数です、Bukkitのバージョンを取得しています。
			String tmp_package[] = Bukkit.getServer().getClass().getPackage()
					.getName().split("\\.");
			...
			Class<?> tmp_ichatBase = getNMSClass("IChatBaseComponent");
//			Class<?> tmp_packetPlayoutEnumtitle = getNMSClass("PacketPlayOutTitle$EnumTitleAction");
			Class<?> tmp_packetPlayoutEnumtitle = getNMSClass("EnumTitleAction");
	...
	public void sendTitle(Player player, String title, String subtitle) {
		try {
			if (title != null) {
				sendPacket(player, constructorTitle.newInstance(enumTitle, methodTitle.invoke(null, "{\"text\":\"" + title+ "\"}")));
			}

			if (subtitle != null) {
				sendPacket(player, constructorTitle.newInstance(enumSubtitle, methodTitle.invoke(null, "{\"text\":\"" + subtitle + "\"}")));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 */
	public void setTime_second(Player player, double feedIn, double titleShow,
			double feedOut) {
		setTime_tick(player, (int) (feedIn * 20), (int) (titleShow * 20),
				(int) (feedOut * 20));
	}
