...
public class SystemListener implements Listener {
	...
	public void joinDungeon(PlayerJoinDungeonGameEvent e) {
		...
		player.sendMessage(getLoginMessage(player));
	...
	public void login(final PlayerJoinEvent e) {
		if (PlayerJoinDungeonGameEvent.inDungeon(e.getPlayer())) {
			PlayerJoinDungeonGameEvent event = new PlayerJoinDungeonGameEvent(e.getPlayer());
	...
	private static String[] getLoginMessage(Player player) {
		if (player.hasPermission("main.lbnDungeonUtil.command.mod.login_msg")) {
			String[] msg = new String[] { ChatColor.RED + "====LOGIN====", ChatColor.AQUA + "以下のコマンドを使うことが出来ます。",
					"/sequence_setblock second blockid:blockdata x y z & x y z & x y z &... ",
					ChatColor.GREEN + "---- 連続でブロックを設置する(/help sequence_setblock で詳細表示)",
					"/sequencecommand second command1 & command2 & command3 &...",
					ChatColor.GREEN + "---- 連続でコマンドを実行する(/help sequencecommand で詳細表示)", "/delaycommand second command",
					ChatColor.GREEN + "---- 一定時間後にコマンドを実行(/help delaycommand で詳細表示)",
					...
					ChatColor.GREEN + "------- (/help timer_command で詳細表示)", ChatColor.RED + "====LOGIN====" };
			return msg;
		} else {
			String[] msg = new String[] { ChatColor.RED + "====LOGIN====", ChatColor.AQUA + "以下のコマンドを使うことが出来ます。",
					"/statusview", ChatColor.RED + "====LOGIN====" };
			return msg;
		}
	...
	public void logout(PlayerQuitEvent e) {
		if (PlayerJoinDungeonGameEvent.inDungeon(e.getPlayer())) {
			PlayerQuitDungeonGameEvent event = new PlayerQuitDungeonGameEvent(e.getPlayer());
			Bukkit.getServer().getPluginManager().callEvent(event);
		}
		loginPlayer--;
	}
	...
	public void PlayerPortalEvent(PlayerPortalEvent e) {
		...
		if (to.getWorld().getName().contains("_the_end") || to.getWorld().getName().contains("_nether")) {
			e.setCancelled(true);
		}
	}
}